...
export default function Collections(data) {
    ...
    post.forEach(function(){
        if(i<8){
            ...
            postList = postList+
            '<article class="blogpost-item my-2 border border-slate-200  rounded-sm p-4"><h2 class="text-xl font-semibold mb-2"><a href='+post[i].link+'>'+post[i].title+'</a></h2><p class="text-gray-500">'+content+'</p>'+
            '<div class="mt-2 flex flex-row items-center justify-between flex-nowrap"><div class="text-sm">'+date+'</div><div><a href="'+post[i].link+'" target="_blank">查看全文</a></div></div></article>'
            i++;
        }
    });
    return (
